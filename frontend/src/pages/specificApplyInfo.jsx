...
const SpecificApplyInfo = () => {
  ...
  useEffect(() => {
    setLoading(true);
    axios
    .get(`http://localhost:5555/api/userRegistration/${id}`)
      .then((response) => {
  ...
  const handleApprove = async () => {
    try {
      ...
      // Make a POST request to the approval endpoint
      const response = await axios.post(`http://localhost:5555/addUser/${id}`);
      
      
        // Handle success, e.g., show a success message or redirect
        const mailResponse = await fetch(
          "http://localhost:5555/api/mail/resetPassword",
          {
            method: "POST",
            headers: {
        ...
        // Handle success, e.g., show a success message or redirect
        console.log("Application approved successfully:", response.data.user);
...
{/* until here */}
      ...
      <Card style={{ backgroundColor: 'white', boxShadow: '0 0 10px rgba(0, 0, 0, 0.1)' }}>
              <Card.Header className="d-flex justify-content-between align-items-center">
                ...
                <div className="text-center">
                  <Image
                   src={`http://localhost:5555/getFiles/hsaas logo.png`}
                    alt="Logo"
                    fluid
  ...
  <div className="row">
    ...
    <a
                            href={`http://localhost:5555/getFiles/${image.apc}`}
                            download={
                              image.apc ? image.apc.split("/").pop() : ""
                            }
                          >
      <img
                              src={`http://localhost:5555/getFiles/${image.apc}`}
                              alt="APC Certificate"
                              width={130}
                              height={100}
                            />
  ...
  <div style={{ position: 'relative', display: 'inline-block' }}>
    <img
     src={`http://localhost:5555/getFiles/${image.photo}`}
      alt='Profile Image'
      width={80}
      height={100}
    />
  ...
  onClick={() => openImageModal(`http://localhost:5555/getFiles/${image.photo}`)}
/>
    </div>
  </div>
</div>
   ...
   <div className="d-flex justify-content-end" >
    <Link to="/sampleTable">
  <button type="submit" className="btn btn-success" onClick={ handleApprove}>
    Approve
  </button>
  </Link>
  <button
...
<Modal show={showImageModal} onHide={closeImageModal}>
    ...
    <Modal show={showSuccessModal} onHide={closeSuccessModal}>
        ...
        <Modal show={showSuccessApproveModal} onHide={() => setShowSuccessApproveModal(false)}>
        <Modal.Header closeButton>
          <Modal.Title>Approval Success!</Modal.Title>
        </Modal.Header>
        <Modal.Body>
          Application approved successfully! Click the button below to view the updated table.
        </Modal.Body>
        <Modal.Footer>
          <Button variant="secondary" onClick={() => setShowSuccessApproveModal(false)}>
            Close
          </Button>
          <Link to="/sampleTable">
